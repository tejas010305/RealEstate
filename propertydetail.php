...
function maskPhone($uphone) {
    return str_repeat('*', strlen($uphone) - 3) . substr($uphone, -3);
}

// Function to mask email
function maskEmail($uemail) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_mail'])) {
    ...
    // Check if the uid and pid are set in the URL
    if (isset($_GET['uid']) && isset($_GET['pid'])) {
        $agentId = $_GET['uid']; // Get agent ID
        $propertyId = $_GET['pid']; // Get property ID

        // Fetch agent details
        $agentQuery = "SELECT * FROM user WHERE uid = '$agentId'";
        $agentResult = mysqli_query($con, $agentQuery);

        if (!$agentResult || mysqli_num_rows($agentResult) == 0) {
            error_log("Error: Agent query failed or agent not found. Query: $agentQuery - " . mysqli_error($con));
            die("<h3>Error: Agent not found.</h3><p>Please verify the agent ID and try again.</p>");
        }

        $row = mysqli_fetch_assoc($agentResult);

        // Fetch property details
        $propertyQuery = "SELECT * FROM property WHERE uid = '$agentId' AND pid = '$propertyId'";
        $propertyResult = mysqli_query($con, $propertyQuery);

        if (!$propertyResult || mysqli_num_rows($propertyResult) == 0) {
            error_log("Error: Agent query failed or agent not found. Query: $propertyQuery - " . mysqli_error($con));
        ...
        if (isset($_SESSION['uname']) && isset($_SESSION['uemail'])) {
            ...
            error_log("Error: Agent query failed or agent not found. Query: - " . mysqli_error($con));
            echo "<script>
                alert('Error: User information not available. Please Login First');
                window.location.href = 'login.php'; // Redirect to login page
            </script>";
            exit();
        }
        
    $id = $_SESSION['uid']; // Get logged-in user ID
    $uid = $_GET['uid'];   // Get Agent ID
    $pid = $_GET['pid']; // Get Property ID

    // Insert request into the 'requests' table
    $sql = "INSERT INTO requests (id,uid, pid, status) VALUES (?, ?, ?, 'Pending')";
    $stmt = $con->prepare($sql);
    $stmt->bind_param("iii", $id, $uid, $pid);
    ...
    if ($stmt->execute()) {
        ...
        $message = "";
        $agentName = $row['uname'];
        $agentEmail = $row['uemail'];
        $agentPhone = $row['uphone'];
        $propertyDetails = "Property Name: " . $property['title'] . "<br>Price: " . $property['price'] . "<br>Location: " . $property['location'];
        ...
        try {
            ...
            $mail->Password = 'iewl tfab gsqt sqpo'; // Replace with your app-specific password
            ...
            // Add CC (Carbon Copy)
            $mail->addCC('getintouchvastuhomes@gmail.com');

            // Use BCC (Blind Carbon Copy) to hide recipient email
            $mail->addBCC($agentEmail, $agentName);

            // Email content
            $mail->isHTML(true);
            $mail->Subject = 'Property Inquiry from ' . $userName;
            $mail->Body = "
                <div style='font-family: Arial, sans-serif; line-height: 1.6;'>
                    <h2 style='color: #4CAF50;'>New Property Inquiry</h2>
                    <p>Dear $agentName,</p>
                    <p>You have received a new inquiry regarding one of your properties. Below are the details:</p>
                    ...
                    <p>If you have any questions, feel free to contact the user directly at their email address.</p>
                    <br>
                    <p style='color: #555;'>Best Regards,</p>
                    <p style='color: #555;'><strong>Vastu Homes Team</strong></p>
                </div>
            ";
            ...
            error_log("Error: Agent query failed or agent not found. Query: $query - " . mysqli_error($con));
            echo "Message could not be sent. Error: {$mail->ErrorInfo}";
        }
    } else {
        error_log("Error: Agent query failed or agent not found. Query: $query - " . mysqli_error($con));
        die("Error: Agent ID or Property ID is missing.");
    }
...
<div id="page-wrapper">
    ...
    <div class="agent-data text-ordinary mt-sm-20">
        ...
        <ul class="mb-3">
            <li><?php echo maskPhone($row['uphone']); ?></li>
            <li><?php echo maskEmail($row['uemail']); ?></li>
        </ul>
        ...
        <?php endif; ?>
                    ...
                    <div class="col-lg-4">
                        ...
                        if (isset($_GET['pid'])) {
                            ...
                            // Create the SQL query to get the map link based on the property ID
                            $sql = "SELECT locMap FROM property WHERE pid = ?";
                            $stmt = $con->prepare($sql);
                            $stmt->bind_param("i", $pid); // 'i' means integer (for pid)
                            $stmt->execute();
                            $result = $stmt->get_result();

                            // Fetch the result
                            if ($row = $result->fetch_assoc()) {
                                $mapLink = $row['locMap']; // Google Maps link stored in the database
                                echo "<html><head><h5>Location</h5><head></html>";
                                echo '<iframe src="' . htmlspecialchars($mapLink) . '" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>';
                            } else {
                                $mapLink = ''; // If no map link is found, set an empty string
                            }
                        } else {
                            // If 'pid' is not provided, you can redirect or show an error
